...
] as const)(`setting $name attribute to $value`, ({ name, value, expected }) => {
  setAttributes(el, { [name]: value })
  expect((el as any)[name]).toBe(expected)
})
...
describe('attributes of an <input type="text">', () => {
  ...
  ] as const)(`"$name" attribute`, ({ name, values, whenRemoved }) => {
    test('setting a value', () => {
      for (const value of values) {
        setAttributes(input, { [name]: value } as any)
        expect((input as any)[name]).toBe(value)
      }
    })

    test('removing the attribute', () => {
      setAttributes(input, { [name]: values[0] } as any)

      removeAttribute(input, name)
      expect((input as any)[name]).toBe(whenRemoved)
    })
  })
})
...
describe.each([
  ...
  ({ type, attribute, values, expectedWhenRemoved }) => {
    test('setting a value', () => {
      ...
      for (const value of values) {
        setAttributes(input, { [attribute]: value } as any)
        expect((input as any)[attribute]).toBe(value)
      }
    })
    ...
    test('removing the attribute', () => {
      const input = document.createElement('input')
      input.type = type
      setAttributes(input, { [attribute]: values[0] } as any)

      removeAttribute(input, attribute)
      expect((input as any)[attribute]).toBe(expectedWhenRemoved)
    })
  }
)
...
describe('the "value" attribute of a <select>', () => {
  ...
  test('setting to a value that exists as an option', () => {
    setAttributes(select, { value: 'foo' } as any)
    expect(select.value).toBe('foo')

    setAttributes(select, { value: 'bar' } as any)
    expect(select.value).toBe('bar')
  })

  test('setting to a value that does not exist as an option', () => {
    setAttributes(select, { value: 'baz' } as any)
    expect(select.value).toBe('')
  })

  test('setting to null', () => {
    setAttributes(select, { value: null } as any)
    expect(select.value).toBe('')
  })
...
describe.each([
  ...
  ({ tag, name, values, whenRemoved }) => {
    test('setting a value', () => {
      ...
      for (const value of values) {
        setAttributes(el, { [name]: value } as any)
        expect((el as any)[name]).toBe(value)
      }
    })

    test('removing it', () => {
      const el = document.createElement(tag)
      setAttributes(el, { [name]: values[0] } as any)

      removeAttribute(el, name)

      expect((el as any)[name]).toBe(whenRemoved)
    })
  }
)
