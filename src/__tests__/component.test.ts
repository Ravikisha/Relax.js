...
describe('Mounting and unmounting', () => {
  test('can be mounted into the DOM', () => {
    ...
    expect(document.body.innerHTML).toBe('<p>A point is that which has no part.</p>')
  })
...
describe('Component state', () => {
  ...
  test('can be based on the props', () => {
    const Comp = defineComponent({
      ...
      render() {
        return h('p', {}, [hString((this as any).state.count)])
      },
    })
...
describe('Component methods', () => {
  test('can use methods to handle events', () => {
    const Comp = defineComponent({
      ...
      increment() {
        ;(this as any).updateState({ count: (this as any).state.count + 1 })
      },
      render() {
        return h(
          'button',
          {
            on: {
              click: (this as any).increment,
            },
          },
          [hString((this as any).state.count)]
        )
      },
    })
    const comp = new Comp()
...
describe('Patching the DOM', () => {
  test('can add event handlers bound to the component', async () => {
      ...
      render() {
        return hFragment([
          (this as any).state.count > 0
            ...
            : null,
          h('span', {}, [hString((this as any).state.count)]),
          h(
            'button',
            {
              id: 'plus-btn',
    ...
    expect(document.body.innerHTML).toBe('<span>0</span><button id="plus-btn">+</button>')
...
describe('Child components', () => {
  ...
  test('the state of children is preserved through a re-render', () => {
    ...
    ;(document.querySelectorAll('li')[0] as HTMLLIElement).click()
    expect(document.body.innerHTML).toBe(
      singleHtmlLine`
      <ul>
...
describe('Events', () => {
  ...
  test('event handlers can be bound to the component', () => {
    ...
    ;(document.querySelectorAll('li')[1] as HTMLLIElement).dispatchEvent(
      new Event('dblclick')
    )
...
describe('Mounted elements', () => {
  test('unmounted components have no mounted elements', () => {
    ...
    expect((comp as any).elements).toEqual([])
  })
  ...
  test('component with a single root element', () => {
    ...
    expect((comp as any).elements).toEqual([expectedEl])
  })
  ...
  const [expectedOne, expectedTwo] = Array.from(document.querySelectorAll('p'))

    expect((comp as any).elements).toEqual([expectedOne, expectedTwo])
  })
  ...
  test('component with a root fragment containing other nested components', () => {
    ...
    const [expectedOne, expectedTwo, expectedThree, expectedFour] = Array.from(
...
describe('Offset', () => {
  test('when the components top-level element is a fragment, it has an offset', () => {
    ...
    const firstComponent = (vdom as any).children[0].component
    const secondComponent = (vdom as any).children[1].component
    const thirdComponent = (vdom as any).children[2].component
...
const PropsComp = defineComponent({
  render() {
    return h('p', { class: (this as any).props.pClass }, [
      'A point is that which has no part.',
    ])
  },
})
...
const ListItem = defineComponent({
  ...
  render() {
    return h(
      'li',
      {
        class: (this as any).state.highlighted ? 'highlighted' : '',
        on: {
          click: () =>
            (this as any).updateState({
              highlighted: !(this as any).state.highlighted,
            }),
          dblclick: () => (this as any).emit('remove-item', (this as any).props.text),
        },
      },
...
const List = defineComponent({
  render() {
    return h(
      'ul',
      {},
      (this as any).props.items.map((item: string) =>
        h(ListItem, {
          text: item,
          on: { 'remove-item': (item: string) => (this as any).emit('remove-item', item) },
        })
      )
    )
  },
...
const DefinitionsComponent = defineComponent({
  ...
  removeItem(item: string) {
    const idx = (this as any).state.items.indexOf(item)
    const items = (this as any).state.items.filter((_: any, i: number) => i !== idx)
    ;(this as any).updateState({ items })
  },
  render() {
    return h(List, {
      items: (this as any).state.items,
      on: { 'remove-item': (this as any).removeItem },
    })
  },
})
