...
test("can't mount an element without a host element", () => {
  ...
  expect(() => mountDOM(vdom, undefined as any)).toThrow()
})
...
test('mount an element with event handlers', () => {
  ...
  ;(vdom.el as HTMLElement | null)?.click()

  expect(onClick).toBeCalledTimes(1)
  expect(onClick).toBeCalledWith(expect.any(MouseEvent))
  expect((vdom as any).listeners).toEqual({ click: expect.any(Function) })
})

test('mounts an element with styles', () => {
  const vdom = h('div', { style: { color: 'red' } })
  mountDOM(vdom, document.body)
  const el = vdom.el as HTMLDivElement
...
test('where there is a host component, the event handlers are bound to it', () => {
  ...
  mountDOM(vdom, document.body, null, comp as any)

  ;(document.querySelector('#btn-1') as HTMLButtonElement).click()
  expect(comp.count).toBe(6)

  ;(document.querySelector('#btn-2') as HTMLButtonElement).click()
...
test('mount a component with props', () => {
  const Component = defineComponent({
    render() {
      return h('p', { class: 'important' }, [(this as any).props.message])
    },
  })
  ...
  if (vdom.type !== 'component') throw new Error('Expected a component vnode')
...
test('mount a component with children', () => {
  const ChildComp = defineComponent({
    render() {
      return h('p', {}, [(this as any).props.message])
    },
  })
  const ParentComp = defineComponent({
    render() {
      return h(
        'div',
        {},
        (this as any).props.messages.map((msg: string) =>
...
test('mount a component with event handlers', () => {
  ...
  const Component = defineComponent({
    render() {
      return h('button', { on: { click: () => (this as any).emit('click') } }, [
        'Click me',
      ])
    },
  })
...
test('mount a component with a parent component', () => {
  ...
  mountDOM(vdom, document.body, null, Parent as any)
...
test('child components keep a reference to their parent component', () => {
  ...
  if (vdom.type !== 'component') throw new Error('Expected a component vnode')
  const compA = vdom.component as any
  const compB = compA.vdom.component as any
  const compC = compB.vdom.component as any
