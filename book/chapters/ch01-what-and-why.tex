...
In this chapter, you will build a mental model for the whole project:

\begin{itemize}
  \item A crisp definition of what a UI runtime must do (model \textrightarrow mount \textrightarrow patch \textrightarrow destroy).
  \item The two execution paths in Relax.js (VDOM baseline and HRBR compiled fast path).
  \item A practical reading workflow: tests are the spec.
\end{itemize}
...
I want to start from a logical place, not from a JS API.
Here is the problem statement that led me to build Relax:
...
Relax.js is my implementation of those responsibilities, with strong emphasis on:

\begin{itemize}
  \item \textbf{Correctness} (tests as a spec)
  \item \textbf{Performance} (avoid unnecessary DOM work)
  \item \textbf{Predictability} (clear scheduling and lifecycle boundaries)
\end{itemize}
...
What makes this repository especially interesting is that it contains \emph{two} rendering strategies:
