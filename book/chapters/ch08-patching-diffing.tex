...
This picture also explains why Chapter~\ref{ch:vnode-model}'s invariant (children are always VNodes) matters: it makes the \texttt{switch} and its helper patchers tractable.
...
Important: \texttt{setAttribute} and \texttt{removeAttribute} are the same helpers discussed in Chapter~\ref{ch:attributes-props} (\texttt{src/attributes.ts}), including the property-vs-attribute rules.
...
\subsection{Events: patchEvents}
	...
	\texttt{patchEvents} compares old and new \texttt{on} objects using \texttt{objectsDiff} and then applies three cases:

\begin{itemize}
  \item \textbf{removed:} remove the existing wrapper listener
  \item \textbf{added:} add a wrapper listener via \texttt{addEventListener} (from \texttt{src/events.ts})
  \item \textbf{updated:} keep the wrapper listener identity, but update its handler/host pointers via \texttt{updateEventListener}
\end{itemize}

\subsection{Benchmark patch-phase breakdown: how to read it}

When you enable the benchmarks' patch-phase collection (\texttt{patchPhases=1}), Relax reports a small table with per-frame averages grouped by phase labels emitted from \texttt{src/patch-dom.js} via \texttt{measurePatchPhase}.
This doesn't change the patcher's semantics; it's a measurement aid to tell you \emph{where} patch time is going.

The phases map to the sections in this chapter:

\begin{itemize}
  \item \texttt{vdom:diff}: time spent computing the child update plan (the sequence diff / keyed reconciliation input).
  \item \texttt{vdom:moves}: time spent performing DOM reorders (inserts/moves) implied by that plan.
  \item \texttt{vdom:attrs}, \texttt{vdom:class}, \texttt{vdom:style}, \texttt{vdom:events}: time spent in the targeted props patchers.
\end{itemize}

As a rule of thumb for optimization work: if \texttt{vdom:diff} dominates, look for ways to reduce child list churn (keys, stable shapes, fewer inserts/removes); if \texttt{vdom:moves} dominates, the DOM work is the bottleneck (reduce reorders, batch updates, avoid layout thrash); and if a props phase dominates, focus on that specific representation (e.g. avoid rebuilding style objects, keep event maps stable, reuse attrs objects so identity fast paths trigger).

This design keeps the DOM listener registry stable during frequent patches, while preserving the key invariant: removal must use the exact same wrapper function object.
...
Relax solves this with the wrapper identity trick from Chapter~\ref{ch:events-dispatcher}. The patcher follows the same rule:

\begin{itemize}
  \item if a handler is removed, remove the old wrapper function
  \item if a handler is added, add a wrapper function
  \item if a handler is updated, mutate the \emph{existing} wrapper to point at the new handler (no detach+attach)
...
\begin{tikzpicture}[
  ...
  \node[box, right=16mm of old] (wrap1) {mount created\\stable wrapper \texttt{w1}};
  \node[box, below=of old] (new) {new props:\\\texttt{on: \{ click: newHandler \}}};
  \node[box, right=16mm of new] (wrap2) {reuse wrapper \texttt{w1},\\update handler pointer};

  \node[box, below=14mm of wrap1] (detach) {detach (removed only):\\\texttt{el.removeEventListener('click', w1)}};
  \node[box, below=of wrap2] (attach) {update (updated only):\\\texttt{updateEventListener(w1, newHandler)}};
...
Tests like \texttt{patch event handlers} effectively specify this invariant: the wrapper identity is stable, and an updated handler changes behavior without requiring re-registration.
...
\subsubsection*{Performance notes (why keyed reordering stays fast)}
Two small details keep the keyed path performant in real workloads:

\begin{itemize}
  \item \textbf{Linear-time anchor tracking:} when applying moves, Relax walks from right to left and tracks the next insertion anchor once, instead of repeatedly scanning to the right (which can create accidental \(O(n^2)\) behavior).
  \item \textbf{Large fully-keyed list heuristic:} when both old and new lists are large and fully keyed, Relax prefers the keyed reorder path early to avoid paying for expensive generic diff script generation. This is a conservative fast path that preserves the same observable DOM result.
\end{itemize}
...
\subsubsection*{How to read the diff algorithm (mental model)}
