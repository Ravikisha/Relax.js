...
I'm going to teach mounting the way I teach any system that mutates state:

\begin{enumerate}
  \item Define the \textbf{contract} (inputs/outputs/failure modes).
  \item Define the \textbf{invariants} we keep true after each step.
  \item Walk the code path \emph{by node type}, because real renderers are dispatchers.
\end{enumerate}

In Relax there's a clean separation:

\begin{itemize}
  \item \textbf{Creation} builds VNode trees (Chapter~\ref{ch:vnode-model}).
  \item \textbf{Mounting} turns VNodes into DOM and stores back-references (this chapter).
  \item \textbf{Patching} reconciles trees (Chapter~\ref{ch:patching-diffing}).
  \item \textbf{Destroying} cleans up listeners / component instances / HRBR blocks (later teardown chapter).
\end{itemize}
...
\begin{itemize}
  ...
  \item If \texttt{vdom.type} doesn't match any known tag in \texttt{DOM\_TYPES}, it throws \texttt{Can't mount DOM of type: ...}.
\end{itemize}

The tests in \texttt{src/\_\_tests\_\_/mount-dom.test.ts} include a couple of ``guardrail'' cases that assert these errors exist.

\subsection*{Two subtle but crucial invariants}
When mounting is correct, these invariants hold and the rest of the runtime can be simpler:

\begin{enumerate}
  \item \textbf{Back-references exist:} every mounted vnode that conceptually ``owns'' a DOM node sets \texttt{vdom.el}.
  \item \textbf{Cleanup data exists:} element vnodes store their concrete listener wrapper functions in \texttt{vdom.listeners}.
\end{enumerate}

Those two fields are what make later patching and destroying possible without extra DOM queries.

\section{The central pattern: dispatch by \texttt{vdom.type}}
	exttt{mountDOM()} is a type-dispatcher. In code it's a \texttt{switch (vdom.type)} that delegates to one helper per vnode kind.

\subsection*{Code: the actual dispatch loop}
This is the heart of \texttt{src/mount-dom.ts}:
...
export function mountDOM(vdom, parentEl, index, hostComponent = null) {
  ...
  switch (vdom.type) {
    ...
    case DOM_TYPES.COMPONENT:
      ...
      break
    case DOM_TYPES.HRBR:
      createHrbrNode(vdom, parentEl, index)
      break
    default:
...
There are two design choices worth noticing right away:

\begin{itemize}
  \item Components schedule \texttt{onMounted()} via the job queue instead of running it inline.
  \item HRBR mounting is isolated behind a host container element so it can be treated as a unit.
\end{itemize}
...
\subsection*{Diagram: mount dispatcher}
This is the mental model for the file \texttt{src/mount-dom.ts}:
...
\begin{tikzpicture}[
  ...
  \node[box] (in) {\textbf{input:} \texttt{vdom} + \texttt{parentEl} + \texttt{index?} + \texttt{hostComponent?}};
  \node[box, below=of in] (sw) {\textbf{switch} (\texttt{vdom.type})};

  \node[box, below left=of sw, xshift=12mm] (text) {TEXT\\\texttt{createTextNode}};
  \node[box, below=of sw] (elem) {ELEMENT\\\texttt{createElementNode}};
  \node[box, below right=of sw, xshift=-12mm] (frag) {FRAGMENT\\\texttt{createFragmentNodes}};
  \node[box, below=of elem, yshift=-2mm] (comp) {COMPONENT\\\texttt{createComponentNode}};
  \node[box, below=of frag, yshift=-2mm] (hrbr) {HRBR\\\texttt{createHrbrNode}};

  \node[box, below=18mm of comp] (out) {\textbf{effects:} inserts DOM + sets \texttt{vdom.el} + stores listeners/components};
...
This is not just an organizational trick: it is what makes patching symmetric later. Chapter~\ref{ch:patching-diffing} will mirror this shape with a patch dispatcher.
...
\subsection*{What the tests prove}
  	texttt{src/\_\_tests\_\_/mount-dom.test.ts} contains two small tests that act like a contract:
...
\subsection{\texttt{addProps}: extracting attrs vs. events}
  	texttt{addProps()} calls \texttt{extractPropsAndEvents(vdom)} (from \texttt{src/utils/props.ts}). The return value is split into:
...
That \texttt{vdom.listeners} back-reference is critical later in teardown: destroying uses it to remove listeners even if the vnode is no longer in the DOM.

\subsection*{Code: attributes vs. events are wired by two different subsystems}
This little helper function is worth memorizing:
...
The important point isn't ``how to set an attribute''; it's \emph{where the boundary is}:

\begin{itemize}
  \item Events are installed through a wrapper so patching can update handlers without leaking listeners.
  \item Attributes are applied directly and are purely data-driven.
\end{itemize}

We'll dive deeper into listener stability in Chapter~\ref{ch:events-dispatcher}.

\subsection*{What the tests prove}
The tests in \texttt{src/\_\_tests\_\_/mount-dom.test.ts} cover a small but meaningful matrix:
...
\section{Event handler binding: the meaning of \texttt{hostComponent}}
  	texttt{mountDOM()} accepts an optional \texttt{hostComponent} argument, which gets threaded into \texttt{createElementNode(...)} and ultimately into \texttt{addEventListeners(...)}.

\subsection*{Diagram: how \texttt{hostComponent} flows into events}
...
\begin{tikzpicture}[
  ...
  \node[box] (mount) {\texttt{mountDOM(vdom, parentEl, index?, hostComponent?)}};
  \node[box, below=of mount] (elem) {ELEMENT path\\\texttt{createElementNode}};
  \node[box, below=of elem] (props) {\texttt{addProps(el, vdom, hostComponent)}};
  \node[box, below=of props] (events) {\texttt{addEventListeners(events, el, hostComponent)}};
  \node[box, below=of events] (handler) {handlers run as\\\texttt{this === hostComponent}};

  \draw[arrow] (mount) -- (elem);
  \draw[arrow] (elem) -- (props);
  \draw[arrow] (props) -- (events);
  \draw[arrow] (events) -- (handler);
\end{tikzpicture}
\end{center}
...
This is a key piece of ``component semantics'' even though it's implemented entirely inside the mounting layer.

\subsection*{Design rationale: the explicit \texttt{hostComponent} thread}
Many frameworks hide handler binding behind closures.
That works, but it makes patching harder because you constantly allocate new function identities.

Relax takes a more systems-y stance:

\begin{quote}
  Event listeners are installed once, and the wrapper decides what \texttt{this} should be.
\end{quote}

That gives you an explicit place to encode semantics (bind to component instance when present) and it makes garbage pressure more predictable.
...
Relax's move is simple:

\begin{quote}
  A fragment vnode stores \texttt{vdom.el = parentEl}.
\end{quote}

That is, a fragment doesn't own a DOM node. Instead, it keeps a reference to the container it was mounted in.

\subsection*{Code: the fragment mount loop and the shifting index}
This loop is the entire reason fragments are a little tricky:
...
function createFragmentNodes(vdom, parentEl, index, hostComponent) {
  ...
  for (const child of children) {
    mountDOM(child, parentEl, idx, hostComponent)
    ...
    switch (child.type) {
      case DOM_TYPES.FRAGMENT:
        ...
        break
      case DOM_TYPES.COMPONENT:
        idx += child.component.elements.length
        break
      default:
        idx++
    }
...
If you want to understand why patching later is careful about indices, start here.
...
But as soon as you insert the first child, the parent's \texttt{childNodes} list changes, and the next child's target index must shift.

Relax solves this by maintaining a local counter \texttt{idx}:

\begin{itemize}
  \item If \texttt{idx == null}, it just appends each child and doesn't care.
...
You'll revisit that in the component lifecycle chapters.

\subsection*{What the tests prove}
  	texttt{src/\_\_tests\_\_/mount-dom.test.ts} contains several important fragment tests:
...
\begin{quote}
  	texttt{enqueueJob(() => vdom.component.onMounted())}
\end{quote}
...
\subsection*{Design rationale: scheduling \texttt{onMounted} instead of calling it directly}
Calling lifecycle hooks synchronously during mount makes it easy to write code that accidentally mutates the tree while you are still building it.
That creates re-entrancy hazards like:

\begin{itemize}
  \item a component mounting triggers state updates
  \item those updates trigger patching
  \item patching now operates on a partially mounted tree
\end{itemize}

The job queue turns that into a two-phase commit:

\begin{enumerate}
  \item Mount builds the DOM and installs listeners.
  \item Jobs run afterward, when invariants are established.
\end{enumerate}

This theme will show up again in scheduling (Chapter~\ref{ch:scheduling}).

\subsection*{What the tests prove}
  	texttt{src/\_\_tests\_\_/mount-dom.test.ts} exercises component creation and parent-child links:
...
The comment in \texttt{src/mount-dom.ts} is worth repeating: \textbf{VDOM owns the host container, not the host's first child.} Patch logic later locates nodes by \texttt{oldVdom.el}; if \texttt{el} pointed inside HRBR output, replacement/removal would get the wrong DOM index.

\subsection*{Code: HRBR host container rationale as an explicit comment}
The implementation makes this policy explicit:

\begin{lstlisting}[language=JavaScript]
function createHrbrNode(vdom, parentEl, index) {
...
\subsection*{What the tests prove}
While the mounting tests live in \texttt{mount-dom.test.ts}, HRBR mounting is exercised via integration tests in \texttt{src/\_\_tests\_\_/hrbr-integration.test.ts}.
...
\subsection*{Code: \texttt{insert()} encodes your DOM insertion policy}
This helper is the "last mile" for every vnode type:
...
If you ever see a renderer bug where nodes show up in the wrong order, \texttt{insert()} and fragment indexing are the first two places I look.

\section{Mini-exercises}
\begin{enumerate}
  \item Add a focused test to \texttt{mount-dom.test.ts} proving that mounting with an out-of-range index appends for element vnodes. (You'll be asserting the same rule \texttt{insert()} implements.)
...
\begin{itemize}
  ...
  \item ``Why schedule effects'' background (React's mental model is a good reference even if implementation differs): \href{https://react.dev/learn/synchronizing-with-effects}{https://react.dev/learn/synchronizing-with-effects}
\end{itemize}
