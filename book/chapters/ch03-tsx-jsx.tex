\chapter{TSX/JSX: an authoring layer on top of \texttt{h()}}

\section{The goal}
Relax.js supports writing UI in TSX/JSX with a React-like syntax.
But I want to demystify it immediately: JSX is only \emph{syntax}.
...
The TSX behavior in this repo is defined in \texttt{tsconfig.json}:
...
\subsection*{Diagram: TSX syntax to VNodes (what TypeScript actually emits)}
\begin{center}
\begin{tikzpicture}[
...
\node[box] (tsx) {\textbf{TSX source}\\\texttt{<Button onClick=... />}\\\texttt{<>...</>}};
\node[box, right=of tsx] (ts) {TypeScript\\\texttt{jsx: react-jsx}\\\texttt{jsxImportSource}};
\node[box, right=of ts] (runtime) {JSX runtime call\\\texttt{jsx(type, props, key)}};
\node[box, below=of runtime] (relaxjsx) {\texttt{relax-jsx/jsx-runtime.ts}\\normalizes props + children};
\node[box, below=of ts, xshift=-10mm] (vnode) {Relax VNodes\\\texttt{h()} / \texttt{hFragment()}};
\node[box, below=of vnode] (mount) {Renderer\\\texttt{mountDOM/patchDOM}};

\draw[arrow] (tsx) -- (ts);
\draw[arrow] (ts) -- (runtime);
\draw[arrow] (runtime) -- (relaxjsx);
\draw[arrow] (relaxjsx) -- (vnode);
\draw[arrow] (vnode) -- (mount);
\end{tikzpicture}
\end{center}

When TSX ``works'' in this repo, it means the right-hand side of this diagram produces the same VNode shapes as writing \texttt{h()} manually.

\section{Two authoring styles you'll see}

\subsection{Direct VDOM authoring (manual \texttt{h()})}
...
\subsection{React-like TSX authoring (compiled via a JSX runtime)}
TypeScript can compile TSX into calls to a configured JSX runtime.
This repo uses a React-like authoring style where:

\begin{itemize}
  \item \texttt{className} maps to Relax's \texttt{class} prop.
  \item \texttt{onClick} maps to Relax's event prop form (internally becomes \texttt{on: \{ click: fn \}}).
...
\begin{quote}
Relax supports both ``native'' Relax props (like \texttt{class} and \texttt{on: \{ click: \ldots \}}) \emph{and} React-like compatibility props (like \texttt{className} and \texttt{onClick}).
\end{quote}
...
\begin{enumerate}
  ...
  \item \texttt{onClick/onInput/\ldots\ \textrightarrow\ on: \{ click/input/\ldots \}}
    \end{itemize}
...
This repo includes \texttt{relax-jsx/jsx-dev-runtime.ts}, which simply re-exports \texttt{Fragment}, \texttt{jsx}, and \texttt{jsxs}, and provides \texttt{jsxDEV(\ldots)} as an alias to \texttt{jsx(\ldots)}.

This is not a performance hack; it's a compatibility bridge so Vite/Vitest can run TSX code in different modes.

\section{Contract surface (the subset is intentional)}
The definitive spec for supported TSX/JSX features lives in:

\begin{itemize}
  \item \texttt{src/\_\_tests\_\_/jsx-tsx.test.ts}
  \item \texttt{src/\_\_tests\_\_/jsx-react-syntax.test.tsx}
\end{itemize}

\subsection{Test 1: TSX can mount via the Relax JSX runtime}
\texttt{src/\_\_tests\_\_/jsx-tsx.test.ts} is the smallest end-to-end proof:
...
\subsection{Test 2: React-like conveniences are supported}
	exttt{src/\_\_tests\_\_/jsx-react-syntax.test.tsx} is a feature matrix.
...
\section{What's not the same as React}
Even though the syntax is familiar, semantics are Relax's:
...
\begin{enumerate}
  \item Add a failing test first in \texttt{src/\_\_tests\_\_/jsx-react-syntax.test.tsx}.
...
Read \texttt{src/\_\_tests\_\_/jsx-react-syntax.test.tsx} and answer:
