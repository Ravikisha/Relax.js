...
\label{ch:vnode-model}

In the previous chapter we treated TSX as ``just syntax'' that boils down to calling \texttt{h()} and \texttt{hFragment()}.
...
\begin{enumerate}
  \item \textbf{Create}: build a VNode tree (usually by calling \texttt{h()} directly, or indirectly via TSX as described earlier).
...
\subsection*{Invariant checklist: identity rules}
