...
\begin{itemize}
  \item \texttt{src/__tests__/}: the VDOM runtime spec.
  \item \texttt{runtime/__tests__/} and \texttt{compiler/__tests__/}: the HRBR + compiler spec.
\end{itemize}
...
The high-level application lifecycle is specified by \texttt{src/__tests__/app.test.ts}.
...
\begin{itemize}
  ...
  \item It asserts DOM structure with \texttt{singleHtmlLine\`...\`}.
\end{itemize}
...
\begin{itemize}
  \item test patterns: \texttt{**/__tests__/**/*.test.ts(x)}
...
This repo solves that in \texttt{src/__tests__/utils.ts} with \texttt{singleHtmlLine()}:

\begin{itemize}
  \item it collapses whitespace
  \item it removes spaces between tags (\texttt{>\s+< -> ><})
  \item it trims
\end{itemize}
...
\begin{itemize}
  \item \texttt{src/__tests__/mount-dom.test.ts}:
    mount behavior, fragment behavior, saving \texttt{vdom.el} references.
  \item \texttt{src/__tests__/patch-dom.test.ts}:
  ...
  \item \texttt{src/__tests__/component.test.ts}:
...
\begin{itemize}
  \item \texttt{src/__tests__/app.test.ts}
  \item \texttt{src/__tests__/component.test.ts}
  \item \texttt{src/__tests__/mount-dom.test.ts}
  \item \texttt{src/__tests__/destroy-dom.test.ts}
\end{itemize}
...
Later, if you want formal citations in PDF output, we can add a bibliography system to \texttt{book/main.tex} and convert these links into \texttt{\textbackslash cite\{...\}} entries.
