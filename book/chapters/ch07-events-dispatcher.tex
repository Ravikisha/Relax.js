...
This is the actual implementation from \texttt{src/events.ts}. The key idea is still the same:

\begin{itemize}
  \item \textbf{The DOM only ever sees the wrapper listener identity.}
  \item The wrapper performs the \texttt{this} binding decision.
\end{itemize}

However, Relax now uses a \textbf{stable wrapper} design: the wrapper function identity stays the same across patches, and patching updates a pointer to the latest user handler.
This avoids remove+add churn in hot update loops.

Notice how the wrapper reads from \texttt{listener.\_\_relaxHandler} and \texttt{listener.\_\_relaxHost} (stored on the wrapper itself):
...
export function updateEventListener(listener, handler, hostComponent = null) {
  if (listener && listener.__relaxStable) {
    listener.__relaxHandler = handler
    listener.__relaxHost = hostComponent
  }
}
...
\paragraph{Why the stable wrapper matters}
In list-heavy UI updates, repeatedly removing+adding listeners is expensive and can dominate frame time.
With a stable wrapper, patches can update handler behavior without touching the DOM listener registry.

This immediately gives Relax three desirable properties:

\begin{itemize}
  \item \textbf{Stable binding semantics:} the wrapper decides whether to call with \texttt{fn.call(hostComponent, event)}.
  ...
  \item \textbf{Efficient updates:} patching can update \texttt{listener.\_\_relaxHandler} in place rather than detach+attach.
\end{itemize}
